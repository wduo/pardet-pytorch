...
class ResNet50(nn.Module):
    ...
    def __init__(self, arch='resnet50', layers=(3, 4, 6, 3),
                 pretrained=True, progress=True, **kwargs):
        super(ResNet50, self).__init__()
        self.model = ResNet(Bottleneck, layers, **kwargs)
        if pretrained:
            state_dict = load_state_dict_from_url(model_urls[arch],
                                                  progress=progress)
            self.model.load_state_dict(remove_fc(state_dict))
